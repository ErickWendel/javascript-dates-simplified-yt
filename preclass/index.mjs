...
const date = new Date("2022-01-25 00:00") // 2022-01-25T03:00:00.000Z
...
{
  const actual = date.toLocaleDateString("pt-br", { ...options, month: 'numeric' })
  const expected = "25/01/2022"
  assert.equal(actual, expected)
}
...
{
  const actual = new Intl.DateTimeFormat('en-US').format(date)
  const expected = '1/25/2022'
  assert.equal(actual, expected)
}
